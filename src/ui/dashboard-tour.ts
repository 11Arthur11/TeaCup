...
function normalizeTarget(value: unknown): TourTarget | undefined {
  ...
  if (
    ...
    const context =
      target.context === "sidebar" || target.context === "workspace"
        ? target.context
        : undefined;
    const mobileContext =
      target.mobileContext === "sidebar" || target.mobileContext === "workspace"
        ? target.mobileContext
        : undefined;
    return {
      type: "selector",
      selector: target.selector.trim(),
      mobileSelector:
        typeof target.mobileSelector === "string" &&
        target.mobileSelector.trim()
          ? target.mobileSelector.trim()
          : undefined,
      context,
      mobileContext,
...
function normalizeConfig(value: unknown): TourConfig {
  ...
  const placements = new Set<TourPlacement>([
    "auto",
    "top",
    "bottom",
    "left",
  ...
  const steps = rawSteps.flatMap((raw, index): TourStep[] => {
    ...
    if (
      ...
      return [];
    const placement = placements.has(step.placement as TourPlacement)
      ? (step.placement as TourPlacement)
      : "auto";
    return [
      {
  ...
  return {
    ...
    storage: {
      registrationPendingKey: stringValue(
        storage.registrationPendingKey,
        DEFAULT_REGISTRATION_KEY,
      ),
      manualPendingKey: stringValue(
        storage.manualPendingKey,
        DEFAULT_MANUAL_KEY,
      ),
...
async function fetchConfig(): Promise<TourConfig> {
  const errors: string[] = [];
  for (const url of configCandidates()) {
    try {
      ...
      if (contentType && !contentType.includes("json")) {
        errors.push(`${url}: invalid content type ${contentType}`);
        continue;
      }
  ...
  throw new Error(
    `Dashboard tour config could not be loaded. ${errors.join(" | ")}`,
  );
}
...
function setMobileSidebar(open: boolean): void {
  ...
  document
    ...
    ?.classList.toggle("app-shell--sidebar-open", open);
}
...
function stepContext(step: TourStep, config: TourConfig): TourContext {
  ...
  if (step.target.type === "selector") {
    if (isMobile(config) && step.target.mobileContext)
      return step.target.mobileContext;
  ...
  if (step.target.type === "sidebar-index") {
    return (
      document.querySelector<HTMLElement>(
        `[data-tour-menu-index="${step.target.index}"]`,
      ) ?? undefined
    );
  }
  const selector =
    isMobile(config) && step.target.mobileSelector
      ? step.target.mobileSelector
...
function positionTour(tour: ActiveTour): void {
  ...
  const cardWidth =
    tour.card.offsetWidth || Math.min(390, window.innerWidth - 24);
  const cardHeight = tour.card.offsetHeight || 250;
  const gap = tour.config.settings.cardGap;
  ...
  let rotate = 0;
  if (placement === "left") {
    pointerX = rect.left - 18;
    rotate = 0;
  }
  if (placement === "right") {
    pointerX = rect.right + 18;
    rotate = 180;
  }
  if (placement === "top") {
    pointerY = rect.top - 18;
    rotate = 90;
  }
  if (placement === "bottom") {
    pointerY = rect.bottom + 18;
    rotate = -90;
  }
...
function stopTour(completed: boolean): void {
  ...
  if (completed)
    storageWrite(
      localStorage,
      tour.config.storage.completedKey,
      String(tour.config.version),
    );
    ...
    if (step.target.type === "center" || target) {
      ...
      if (target) {
        ...
        target.scrollIntoView({
          behavior: tour.config.settings.scrollBehavior,
          block: "nearest",
          inline: "nearest",
        });
      ...
      tour.card.innerHTML = `
        ...
        <div class="dashboard-tour__actions">
          ...
          <span></span>
          <button type="button" class="button button--primary" data-tour-next>${icon(index === tour.config.steps.length - 1 ? "check" : "arrow_forward")} ${escapeHtml(index === tour.config.steps.length - 1 ? tour.config.ui.finishLabel : tour.config.ui.nextLabel)}</button>
          <button type="button" class="button button--secondary" data-tour-previous ${index === 0 ? "disabled" : ""}>${escapeHtml(tour.config.ui.previousLabel)} ${icon("arrow_back")}</button>
        </div>`;
      await new Promise<void>((resolve) =>
...
async function startTour(config: TourConfig): Promise<void> {
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      if (element?.closest("[data-tour-cancel]")) {
        stopTour(false);
        return;
      }
      if (element?.closest("[data-tour-previous]")) {
        void renderStep(tour, tour.stepIndex - 1, -1);
        return;
      }
  ...
  window.addEventListener("resize", () => positionTour(tour), {
    passive: true,
    signal: controller.signal,
  });
  window.addEventListener("scroll", () => positionTour(tour), {
    passive: true,
    signal: controller.signal,
    capture: true,
  });
  window.addEventListener("popstate", () => stopTour(false), {
    signal: controller.signal,
  });
...
async function startPendingTour(): Promise<void> {
  ...
  try {
    ...
    const registered =
      storageRead(localStorage, config.storage.registrationPendingKey) ||
      storageRead(localStorage, DEFAULT_REGISTRATION_KEY);
    const manual =
      storageRead(sessionStorage, config.storage.manualPendingKey) ||
...
export function scheduleDashboardTourAutoStart(): void {
  ...
  requestAnimationFrame(() =>
    requestAnimationFrame(() => void startPendingTour()),
  );
}
